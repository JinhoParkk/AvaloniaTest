...
namespace AvaloniaApplication1.ViewModels.JinoOrder;

public partial class JinoOrderMainViewModel : ViewModelBase
{
    private readonly IJinoOrderService _orderService;
    ...
    public JinoOrderMainViewModel() : this(new MockJinoOrderService(), null)
    {
    }

    public JinoOrderMainViewModel(IJinoOrderService orderService, IPlatformInfo? platformInfo)
    {
    ...
    {
        var items = await _orderService.GetMenuItemsAsync();
    ...
    {
        TodaySummary = await _orderService.GetDailySummaryAsync(DateTime.Today);
        var popular = await _orderService.GetPopularMenuItemsAsync(
    ...
    private async Task SelectCategory(MenuCategory category)
    {
        ...
        if (category != null)
        {
            var items = await _orderService.GetMenuItemsByCategoryAsync(category.Id);
            MenuItems = new ObservableCollection<MenuItem>(items);
        }
    }
    ...
    private async Task ToggleSoldOut(MenuItem item)
    {
        ...
        var success = await _orderService.ToggleMenuItemSoldOutAsync(item.Id, !item.IsSoldOut);
        if (success)
        {
    ...
    private async Task ToggleAvailable(MenuItem item)
    {
        ...
        var success = await _orderService.ToggleMenuItemAvailabilityAsync(item.Id, !item.IsAvailable);
        if (success)
        {
